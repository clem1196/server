...
module.exports = {
  validarAddUser: (req, res, next) => {
    if (!req.body.nombre_usuario || req.body.nombre_usuario.length < 3) {
      return res
        .status(400)
        .send({ Message: "Por favor ingrese un nombre mínimo 3 caracteres" });
    }
    if (!req.body.contraseña || req.body.contraseña.length < 6) {
      return res
        .status(400)
        .send({ Message: "Por favor ingrese contraseña mínimo 6 caracteres" });
    }
    //repite_contraaseña es solo para comprobar que la contraseña sea correcta
    if (
      !req.body.repite_contraseña ||
      req.body.repite_contraseña != req.body.contraseña
    ) {
  ...
  validarUsuarios_roles: (req, res, next) => {
    if (
      !req.body.idusuario ||
      !req.body.idusuario > 0 ||
      !Number.isInteger(parseInt(req.body.idusuario))
    ) {
      return res
        .status(400)
        .send({ Message: "Ingrese un numero entero mayor que cero" });
    }
    if (
      !req.body.idroles ||
      !req.body.idroles > 0 ||
      !Number.isInteger(parseInt(req.body.idroles))
    ) {
      return res
        .status(400)
        .send({ Message: "Ingrese un numero entero mayor que cero" });
  ...
  isLoggedIn: (req, res, next) => {
    ...
    if (!token) {
      return res
        ...
        .send({
          ok: false,
          Message: "El token no es válido o no existeeee",
          token: token,
        });
    } else {
      token = token.split(" ");
      if (token.length > 1) {
        ...
        jwt.verify(token, "clemente", (err, token) => {
          if (err)
            res
              .status(401)
              .send({ ok: false, Message: "No se pudo comprobar el token" });
          req.token = token;
          next();
        });
      } else {
  ...
  isAdmin: (req, res, next) => {
    const rolAdmin = req.token.rol.filter(
      (element) => element.nombre_rol === "admin"
    );
    if (rolAdmin.length > 0) return next();
    res.status(401).send({ Message: "usted no tien permiso" });
  },
  //Rutas para ser modificado: Add, Edit, Delete
  //al modulo ventas tiene acceso el vendedor y el admin
  isVendedor: (req, res, next) => {
    const rolVendedor = req.token.rol.filter(
      (element) =>
        element.nombre_rol === "vendedor" || element.nombre_rol === "admin"
    );
    if (rolVendedor.length > 0) return next();
    res.status(401).send({ Message: "usted no tien permiso" });
  },
  //al modulo contabilidad tiene acceso el contador, y el admin
  isContador: (req, res, next) => {
    const rolContador = req.token.rol.filter(
      (element) =>
        element.nombre_rol === "contador" || element.nombre_rol === "admin"
    );
    if (rolContador.length > 0) return next();
    res.status(401).send({ Message: "usted no tien permiso" });
  },
  //al modulo empleado tiene acceso el contador, y el admin
  isEmpleado: (req, res, next) => {
    const rolEmpleado = req.token.rol.filter(
      (element) =>
        element.nombre_rol === "empleado" || element.nombre_rol === "admin"
    );
    if (rolEmpleado.length > 0) return next();
    res.status(401).send({ Message: "usted no tien permisooooo" });
  },

  //Rutas para listas admin y auditor

  ///el auditor tiene acceso a todo el sistema pero no puede hacer modificaciones
  isAuditor: (req, res, next) => {
    const rolAuditor = req.token.rol.filter(
      (element) =>
        element.nombre_rol === "auditor" || element.nombre_rol === "admin"
    );
    if (rolAuditor.length > 0) return next();
    res.status(401).send({ Message: "usted no tien permiso" });
  },

  //Rutas para listar usuario, auditor y admin
  isVenAuAdm: (req, res, next) => {
    const rolVendedor = req.token.rol.filter(
      (element) =>
        element.nombre_rol === "vendedor" ||
        element.nombre_rol === "auditor" ||
        element.nombre_rol === "admin"
    );
    if (rolVendedor.length > 0) return next();
    res.status(401).send({ Message: "usted no tien permiso" });
  },
  //al modulo contabilidad tiene acceso el contador, y el admin
  isConAutAdm: (req, res, next) => {
    const rolContador = req.token.rol.filter(
      (element) =>
        element.nombre_rol === "contador" ||
        element.nombre_rol === "auditor" ||
        element.nombre_rol === "admin"
    );
    if (rolContador.length > 0) return next();
  ...
  //al modulo empleado tiene acceso el contador, y el admin
  isEmpAudAdm: (req, res, next) => {
    const rolEmpleado = req.token.rol.filter(
      (element) =>
        element.nombre_rol === "empleado" ||
        element.nombre_rol === "auditor" ||
        element.nombre_rol === "admin"
    );
    if (rolEmpleado.length > 0) return next();
  ...
  doNotRemoveRolAdmin: async (req, res, next) => {
    ...
    console.log(roles[0]);
    if (roles[0].length > 0) {
      if (roles[0][0].nombre_rol != "admin") return next();
      //caso contrario
      res
        .status(401)
        .send({ Message: "No debe eliminar ni editar al rol admin" });
    } else {
  ...
  doNotRemoveUserAdmin: async (req, res, next) => {
    ...
    const usuarioRol = await pool.query(
      "call get_usersNames_rolesNames_ById(?)",
      [id]
    );
    if (usuarioRol[0].length > 0) {
      ...
      if (usuarioRol[0][0].nombre_rol === "admin") {
        ...
        const usuariosRoles = await pool.query(
          "call get_usersNames_rolesNames()"
        );
        //y filtramos por el nombre_rol===admin
        const rolAdmin = usuariosRoles[0].filter(
          (element) => element.nombre_rol === "admin"
        );
        if (rolAdmin.length > 1) {
          ...
          res
            .status(401)
            .send({
              Message: "No debe eliminar ni editar a la relacion admin",
            });
        }
      } else {
